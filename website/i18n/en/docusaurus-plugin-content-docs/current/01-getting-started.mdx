...
`@dodge164/react-yandex-maps` is a wrapper around Yandex.Maps API that allows you to
...
```bash
npm install --save @dodge164/react-yandex-maps
```
...
```jsx
import { YMaps } from '@dodge164/react-yandex-maps';
...
```jsx
import { YMaps, Map } from '@dodge164/react-yandex-maps';
...
way to stop Yandex. Map objects from updating completely with the library API.
